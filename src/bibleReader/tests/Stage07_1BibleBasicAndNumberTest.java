...
public class Stage07_1BibleBasicAndNumberTest {
	...
	public void setUp() throws Exception {
		...
		VerseList copyOfVerseList = new VerseList(versesFromKJVFile.getVersion(), versesFromKJVFile.getDescription(),
				copyOfList);
		...
		// Now make a copy of the VerseList
		copyOfVerseList = new VerseList(versesFromESVFile.getVersion(), versesFromESVFile.getDescription(), copyOfList);
...
//----------------------------------------------------
	...
	public void testGetLastVerseNumber() {
		...
		assertEquals(21, KJVBible.getLastVerseNumber(BookOfBible.Revelation, 22));
	...
	// Helper methods.
	/**
	 * This is not an efficient method at all, but it is more efficient than doing
	 * it by hand. It does a linear search to find the starting and ending verses.
	 * It also assumes that firstVerse comes before lastVerse and that both are in
	 * the ArrayList. In other words, this method is only useful for passage lookups
	 * that are expected to succeed.
	 * 
	 * @param firstVerse    The first verse in the passage (inclusive)
	 * @param secondVerse   The last verse in the passage (inclusive)
	 * @param actualResults The list of verses from the passage that is hopefully
	 *                      all of those between firstVerse and lastVerse,
	 *                      inclusive.
	 */
	 ...
	 * @param actualResults
	 */
	public void compareReferenceArrayWithExpected(Reference firstVerse, Reference lastVerse,
			ArrayList<Reference> actualResults) {
		int i = 0;
		while (!versesFromKJVFile.get(i).getReference().equals(firstVerse)) {
			i++;
		}
